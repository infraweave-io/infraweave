...
mod cli_http_tests {
    ...
    /// Kill any leftover Docker containers that are occupying the ports we need
    /// (e.g. from a previous crashed test run).
    ...
    async fn ensure_server() -> u16 {
        *SERVER
            .get_or_init(|| async {
                // The scaffold's bootstrap uses relative paths like
                // "integration-tests/providers/aws-5" — set CWD to workspace root.
                ...
                // Leak the infra guard so containers stay alive for the whole test run.
                ...
                // Point http_client at the in-process server.
                ...
                // Write a tokens.json with the "local" sentinel so http_client
                // skips JWT validation and the server (feature=local) allows
    ...
    async fn test_http_list_deployments_empty() {
        ...
        let deployments = http_client::http_get_deployments("000000000000", "us-west-2")
            .await
