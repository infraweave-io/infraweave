    ...
    .await
    {
        Ok(command_result) => {
            ...
            match upload_file_to_change_records(handler, &output_json_key, output_json).await {
                Ok(_) => {
                    println!(
                        "Successfully uploaded show output file ({})",
                        output_filename
                    );
                }
                Err(e) => {
                    println!("Failed to upload show output file: {}", e);
                }
            }
...
pub async fn record_apply_destroy_changes(
    ...
    match upload_file_to_change_records(handler, &apply_raw_json_key, &raw_plan_json).await {
        Ok(_) => {
            println!("Successfully uploaded apply output file");
        }
        Err(e) => {
            println!("Failed to upload apply output file: {}", e);
        }
    }
    ...
    let infra_change_record = InfraChangeRecord {
        ...
        plan_raw_json_key: apply_raw_json_key,
