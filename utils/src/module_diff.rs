...
pub fn diff_modules(module1: &str, module2: &str) -> (Vec<ModuleDiffAddition>, Vec<ModuleDiffChange>, Vec<ModuleDiffRemoval>) {
...
fn diff_values(
    ...
    value2: &JsonValue,
    path: &str,
) -> (Vec<ModuleDiffAddition>, Vec<ModuleDiffChange>, Vec<ModuleDiffRemoval>) {
    ...
    match (value1, value2) {
        (JsonValue::Object(map1), JsonValue::Object(map2)) => {
            ...
            for (key, val1) in map1 {
                let new_path = if path.is_empty() { format!("/{}", key) } else { format!("{}/{}", path, key) };
