...
pub async fn get_user_delegation_key(
    ...
    let delegation_key_url = reqwest::Url::parse(&format!(
        "https://{}.blob.core.windows.net/?restype=service&comp=userdelegationkey",
        storage_account
    ))
    .map_err(|e| anyhow!("Invalid storage account URL: {}", e))?;

    if delegation_key_url.scheme() != "https" {
        return Err(anyhow!("Delegation key URL must use HTTPS"));
    }
