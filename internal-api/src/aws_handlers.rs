...
pub async fn publish_stack(payload: &Value) -> Result<Value> {
    use env_common::logic::{upload_module, upload_provider_cache};
    use env_defs::{CloudProvider, ModuleResp};
    ...
    info!(
        "Uploading stack {} version {} to all regions",
        module.module, module.version
    );
    ...
    for region in all_regions.iter() {
        ...
        upload_module(&region_handler, &module, &zip_base64.to_string())
            .await
            .map_err(|e| anyhow!("Failed to upload stack to region {}: {}", region, e))?;
    ...
    info!("Stack uploaded successfully to all regions");
