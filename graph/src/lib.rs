...
fn create_node(
    ...
    // We already do this later for vars/locals/etc but let's be stricter here for data/resources
    // IF it's not in resource_map, and not in active_plan_addresses...
        ...
        if let Some(state_val) = state_values_map.get(&address) {
            ...
            if address.starts_with("output.") {
                ...
                if let Some(change) = output_map.get(key) {
                    ...
                    // Need to check if output exists in state map? Output map is from changes.
                    // State outputs are usually top-level in StateValues, not processed here per-resource recursion.
                    // But we can check if we have a state value for it if we passed a separate map or just handle it here if it's in resource map somehow?
                    // No, outputs are separate. Let's stick to existing logic for outputs unless we have them in state.
                    // But output changes logic was: if not in map -> no-op.
...
fn strip_attribute_path(reference: &str) -> String {
    ...
    // For module references (module.name.output_name), keep all parts
    if parts[0] == "module" || parts[0] == "var" || parts[0] == "local" || parts[0] == "data" {
...
fn traverse_configuration(
    ...
    if let Some(resources) = &module.resources {
        for res in resources {
            // The address in configuration is usually the full address if at root,
            // or we might need to construct it?
            // "address": "null_resource.disabled_example"
            // Let's assume 'address' provides the unique ID if we are looking at the right module level,
            // OR we use our calculated path.
            // Actually, `address` in `ResourceConfig` is likely "null_resource.foo", not "module.x.null_resource.foo".
            // Let's construct it to be safe/correct.
            // address usually contains Type.Name.

            // Wait, if res.address is "null_resource.foo", and we are in module "mod",
            // the full address is "module.mod.null_resource.foo".
            ...
            if let Some(exprs) = &res.expressions {
                for (arg_name, expr_val) in exprs {
                    ...
                    for ref_target in references {
                        // ref_target is e.g. "aws_instance.web.id" or "var.region"
                        // We need to map this to the Dependency NODE ID.
                        // The dependency might be:
                        // 1. A resource: "aws_instance.web"
                        // 2. A module output: "module.db.output_addr" ?
                        // 3. A var: "var.region"

                        // We don't know the exact ID of the target node yet (simplification might happen),
                        // but we can try to guess the most likely resource ID by stripping attributes.
                        // e.g. "aws_instance.web.id" -> "aws_instance.web"
                        // "module.db.id" -> "module.db" (group?) or "module.db" (resource inside?)

                        // Heuristic:
                        // If it starts with "var.", "local.", "data.", take 2 parts usually.
                        // If it is a resource type, take 2 parts.
                        // If it is module, "module.name.output" -> ?

                        // For now, store the raw reference target or a cleaned version.
                        // The edge matching later will be fuzzy:
                        // If Edge(Source=full_address, Target=T) exists, check if T matches ref_target prefix.
...
pub fn process_graph(
    ...
    let index_strip_regex = Regex::new(r"\[[^\]]+\]$").unwrap();
    ...
    // 2. Parse DOT File
    let node_regex = Regex::new(r#"^[\t\s]*"(.+?)"\s*\[label\s*=\s*"(.+?)""#).unwrap();
    let edge_regex = Regex::new(r#"^[\t\s]*"(.+?)"\s*->\s*"(.+?)""#).unwrap();
    ...
    eprintln!("DEBUG: Found {} config_deps total", config_deps.len());
    let output_config_deps: Vec<_> = config_deps
        .iter()
        .filter(|((dep, _), _)| dep.starts_with("output.") || dep.contains(".output."))
        .collect();
    eprintln!(
        "DEBUG: Found {} output config_deps",
        output_config_deps.len()
    );
    for ((dep, tgt), _) in &output_config_deps {
        eprintln!("DEBUG: Output dep: {} -> {}", dep, tgt);
    }
    ...
    for ((dependent, ref_target), _) in &config_deps {
        ...
        if dependent.starts_with("output.") || dependent.contains(".output.") {
            ...
            let normalized_target = if qualified_target.starts_with("module.") {
                ...
                // Check if this looks like a resource reference (has a resource type like aws_, azurerm_, etc.)
                // Resource refs: module.eks.module.eks.aws_eks_cluster.this
                // Output refs: module.eks.module.eks.cluster_endpoint
                let is_resource_ref = parts.iter().any(|p| {
                    p.starts_with("aws_")
                        || p.starts_with("azurerm_")
                        || p.starts_with("google_")
                        || p.starts_with("data.")
                        || p == &"data"
                });

                if !is_resource_ref && parts.len() >= 3 && !parts.contains(&"output") {
