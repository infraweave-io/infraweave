...
impl VariableChange {
    ...
    pub fn compute(before: Option<&Value>, after: &Value) -> Option<Self> {
        ...
        // Always return a VariableChange object to show what variables exist,
        // even if they're all unchanged (for visibility and audit trail)
        Some(Self {
...
mod tests {
    ...
    fn test_variable_change_no_changes() {
        ...
        // Should return VariableChange with only unchanged variables
        assert!(result.added.is_none());
        assert!(result.removed.is_none());
        assert!(result.changed.is_none());
        assert!(result.unchanged.is_some());
        ...
        let unchanged = result.unchanged.unwrap();
        assert_eq!(unchanged["instance_type"], "t2.micro");
        assert_eq!(unchanged["region"], "us-west-2");
    }
    ...
    fn test_variable_change_empty_objects() {
        ...
        // Should return VariableChange with all fields None when no variables exist
        assert!(result.added.is_none());
        assert!(result.removed.is_none());
        assert!(result.changed.is_none());
        assert!(result.unchanged.is_none());
    ...
    fn test_variable_change_only_unchanged_returns_none() {
        ...
        // Returns VariableChange with unchanged variables populated
        assert!(result.added.is_none());
        assert!(result.removed.is_none());
        assert!(result.changed.is_none());
        assert!(result.unchanged.is_some());
        ...
        let unchanged = result.unchanged.unwrap();
        assert_eq!(unchanged["var1"], "value1");
        assert_eq!(unchanged["var2"], "value2");
    }
}
